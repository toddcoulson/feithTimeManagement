app.controller('AllTimesheetsController', ['$routeParams', '$modal', '$rootScope',
                                           ...
                                           function ($routeParams, $modal, $rootScope, $scope, $q, $http,
                                                      $filter, GetNewIdService, coreService, projectService, timeEntryService,
                                                      prefsService) {
                                               ...
                                               $scope.getProjectDetails = function(){
                                                   $scope.prs.getProjectDetails($scope.project.project_id).then(function(){
                                                       console.log("Still getting project details. ")
                                                       $scope.project = $scope.prs.projectObject();
                                                   });
                                               }
                                               $scope.getTimeEntries = function(min, max){
                                                   timeEntryService.getTimeEntriesAll(min, max).then(function(entries){
                                                       console.log(entries)
                                               ...
                                               $rootScope.$on("getEditedModal", function(event, newTE){
                                                   angular.forEach($scope.entries , function(e) {
                                                       console.log(e.time_entry_id, $scope.entryValue.time_entry_id)
    ...
    var editTimeModalInstance = $modal.open({
        template: '<time-entry editing="editing" close-modal="cancel()" editobject="
